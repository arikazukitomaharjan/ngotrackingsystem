...
<h1 class="list_title">All Organizations<a href="{{ URL::route('createOrganization') }}" class="app_btn dark_grey_box">Add New</a></h1>
...
?>
  ...
  <tr>
    ...
    <span class="icon_green"> <a href="{{$orgLink}}"> <i class="icon-eye"></i> </a></span>
...
<tfoot>
  ...
  <tr>
    ...
    <td colspan="2">showing {{ $dataList->currentPage() }} of {{ $dataList->lastPage() }}</td>
    <td colspan="1" align="right">
    show <span>
        <select id="row-view-select">
        
            <option {{ $view==10?'selected':'' }} value="{{ url('organizations?view=10') }}">10</option>
            <option {{ $view==25?'selected':'' }} value="{{ url('organizations?view=25') }}">25</option>
            <option {{ $view==50?'selected':'' }} value="{{ url('organizations?view=50') }}">50</option>
            <option {{ $view==100?'selected':'' }} value="{{ url('organizations?view=100') }}">100</option>
        </select>
    </span>
    </td>
